...
extension FrameConnect {
    ...
    func properties() -> [UInt8] {
        ...
        if let sessionExpiryInterval = self.sessionExpiryInterval {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.sessionExpiryInterval.rawValue, value: sessionExpiryInterval.byteArrayLittleEndian)
        }

        // 3.1.2.11.3 Receive Maximum
        if let receiveMaximum = self.receiveMaximum {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.receiveMaximum.rawValue, value: receiveMaximum.hlBytes)
        }

        // 3.1.2.11.4 Maximum Packet Size
        if let maximumPacketSize = self.maximumPacketSize {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.maximumPacketSize.rawValue, value: maximumPacketSize.byteArrayLittleEndian)
        }

        // 3.1.2.11.5 Topic Alias Maximum
        if let topicAliasMaximum = self.topicAliasMaximum {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.topicAliasMaximum.rawValue, value: topicAliasMaximum.hlBytes)
        }

        // 3.1.2.11.6 Request Response Information
        if let requestResponseInformation = self.requestResponseInformation {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.requestResponseInformation.rawValue, value: [requestResponseInformation])
        }
        // 3.1.2.11.7 Request Problem Information
        if let requestProblemInfomation = self.requestProblemInfomation {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.requestProblemInfomation.rawValue, value: [requestProblemInfomation])
        }
        // 3.1.2.11.8 User Property
        if let userProperty = self.userProperties {
            ...
            for (value) in dictValues {
                properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.userProperty.rawValue, value: value.bytesWithLength)
            }
        }
        // 3.1.2.11.9 Authentication Method
        if let authenticationMethod = self.authenticationMethod {
            properties += CocoaMQTTProperty(type: CocoaMQTTPropertyType.authenticationMethod.rawValue, value: authenticationMethod.bytesWithLength)
        }
        // 3.1.2.11.10 Authentication Data
